...
function suappFreezeHeader1(_sugrids) {
    try {
        if (_sugrids.length > 0) {
            ...
            _.each(_sugrids, function (i, j) {
                ...
                if (_headerControlHTML) {
                    ...
                    var _GTRTH = "<div class='fixedheader' id='" + i + "_fixed' style='position: absolute; top: 0px; left: 1px; width: 100%; overflow: hidden;z-index: 9;'><table width='" + tblclassW + "' class='" + tblclass + "' cellspacing='0' cellpadding='0' border='0'><thead>" + _headerControlHTML + "</thead></table></div>"
...
function SetTabletdFreeze(ctrlId) {
    $('#' + ctrlId + ' thead tr:nth-child(2) th').each(function (i, j) {
        var colwidth = $(this).outerWidth();
        $('#' + ctrlId + '_fixed thead tr:nth-child(2) th').each(function (k, l) {
            if (i == k) {
