...
var w2utils = (function ($) {
    ...
    var obj = {
        ...
        settings: {
            ...
            "dateStartYear": 1947,       // start year for date-picker
            "dateEndYear": new Date().getFullYear() + 100 // end year for date picker
        },
        isBin: isBin,
        isInt: isInt,
        isFloat: isFloat,
...
(function ($) {
    ...
    w2grid.prototype = {
        ...
        request: function (cmd, add_params, url, callBack) {
            ...
            ajaxOptions.data.gridname =  this.name;
            ajaxOptions.data.selected = "";
            ajaxOptions.data.search = ajaxOptions.data.search || [];
            ajaxOptions.data.sort = ajaxOptions.data.sort || [];
        ...
        requestComplete: function(status, cmd, callBack) {
            ...
            if (status != 'error') {
                ...
                if (responseText != null && responseText != '') {
                    ...
                    var __REMOVE__ = data;
                    data={}
                    data.records = __REMOVE__.d[0];
                    data.status = 'success';
                    data.total = __REMOVE__.d[1];
                     var bindColumns = []; var gridColGroups = [];
                    
                   //if(this.bindColumns== "" || this.bindColumns== undefined){
                    bindColumns = []
                    if( data.records.length>0){
                         for (var keyss in data.records[0]) {
                         var collazy;
                             if(keyss=="PF_TRAN_DET_ID"){
                                 collazy = { field: keyss, caption: toTitleCase(keyss).split('-')[0], size: '102px', sortable: false, resizable: true, arrow: true, searchable: 'int',hidden:true }
                             }
                              else  if(keyss=="PACKING" || keyss=="SNO" || keyss=="ITEM_NAME" || keyss=="ITEM_CD" ||keyss=="AMOUNT"){
                                 collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '60px', sortable: false,frozen: true, resizable: true, arrow: true, searchable: 'text' ,style: "text-align:right"}
                             }
                             else if(keyss=="APPROVED_AMOUNT"){
                             if(data.records[0].IS_STATEMENT=="N")
                             {
                                 if(data.records[0].RECORD_STATUS=="A" || data.records[0].TRAN_TYPE=="PD" || data.records[0].TRAN_TYPE=="C"){
                                    collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, frozen: true,searchable: 'text',style: "text-align:right" }
                                 }
                                 else{
                                     collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, frozen: true,searchable: 'text',editable: { type: 'float'}, style: "text-align:right" }
                                 }
                               }
                               else{
                                      collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, frozen: true,searchable: 'text',style: "text-align:right" }
                             ...
                             else if(keyss=="NET_AMOUNT"){
                                    collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '102px', sortable: false, resizable: true, arrow: true,searchable: 'text',style: "text-align:right" }
                             }
                             else if(keyss=="BILL_AMOUNT" || keyss=="DOCTOR_PCT"){
                                    collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '102px', sortable: false, resizable: true, arrow: true,searchable: 'text',style: "text-align:right" }
                             }
                             else if(keyss=="CONCESSION_AMOUNT"){
                                    collazy = { field: keyss, caption: '<div style="text-align:right;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '102px', sortable: false, resizable: true, arrow: true,searchable: 'text',style: "text-align:right" }
                             }
                             else  if(keyss=="APPROVED_COMMENTS"){

                             if(data.records[0].IS_STATEMENT=="N"){
                                 if(data.records[0].RECORD_STATUS=="A"){
                                    collazy = { field: keyss, caption: '<div style="text-align:left;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '140px', sortable: false, resizable: true, arrow: true,frozen: true, searchable: 'text' }
                                 }
                                 else{
                                    collazy = { field: keyss, caption: '<div style="text-align:left;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '140px', sortable: false, resizable: true, arrow: true,frozen: true, searchable: 'text',editable: { type: 'text'} 
                                 }                                 
                                 if(keyss=="APPROVED_COMMENTS"){
                                     bindColumns.push(collazy);
                                     collazy = { field: keyss, caption: '<div style="text-align:left;font-weight:bold">update</div>', size: '60px', sortable: false, resizable: false, arrow: true,frozen: true, searchable: 'text',
                                            render:function(record){
                                                return '<input type="button" style="width=70px; cursor: pointer;background: #01a2d8;" title="Update" class="myupdate" value="Update"/>'                       
                                            }
                                     }
                                  }
                             }
                             }else{
                             collazy = { field: keyss, caption: '<div style="text-align:left;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '200px', sortable: false, resizable: true, arrow: true,frozen: true, searchable: 'text' }
                             
                             }
                           }
                           else  if(keyss=="BILL_ID"){
                                     collazy = { field: keyss, caption: '<div style="text-align:left;font-weight:bold">History</div>', size: '60px', sortable: false, resizable: false, arrow: true,frozen: true, searchable: 'text',
                                            render:function(record){
                                                return '<input type="button" style="width=70px; cursor: pointer;background: #01a2d8;" title="History" class="myhistory" value="History"/>'                       
                                            }
                                     }
                           }
                            else if(keyss=="PATIENT_NAME"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '200px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             } 
                             else if(keyss=="TRAN_TYPE"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             } 
                            else if(keyss=="COMPANY_NAME"){
                                collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                            } 
                            else if(keyss=="CONSULTATION_TYPE_NAME"){
                                collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '170px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                            } 
                            else if(keyss=="WARD_GROUP_NAME"){
                                collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '150px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                            } 
                            else if(keyss=="SERVICE_NAME"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '300px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }                           
                             else if(keyss=="PRICE_CATEGORY_NAME"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '250px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }      
                              else if(keyss=="SERVICE_CLASS_NAME"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '200px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }                           
                             else if(keyss=="REMARKS"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '200px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }           
                                                     
                             else if(keyss=="SURGERY_NAME"){
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '300px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }                           
                             else {
                                 collazy = {field: keyss, caption:  '<div style="text-align:center;font-weight:bold">' + toTitleCase(keyss).split('-')[0] + '</div>', size: '102px', sortable: false, resizable: true, arrow: true, searchable: 'text' }
                             }
                        ...
                        this.columns = bindColumns;
                         this.summary.push(this.records);
