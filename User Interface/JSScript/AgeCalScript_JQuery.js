...
function OnGetAge() {
    if (new Date($('.mydate').val()) > new Date()) {
        $(".stoast").toastText("Info", "You cannot select a day after today!", 2, 3);
        ...
        $(_pFix + 'newAgeUc_txtDob').val('');
        $(_pFix + 'newAgeUc_txtDay').val('');
        $(_pFix + 'newAgeUc_txtYear').val('');
        $(_pFix + 'newAgeUc_txtMonths').val('');
        return false;
    }

    else {

        var _sele_date = new Date($('.mydate').val()).format('dd-MM-yyyy'); ;  /*obj.get_selectedDate().format("dd-MM-yyyy");*/
...
function fn_CalAge(ctrlY, ctrlM, ctrlD, ctrlDOB, ctrlHY, ctrlHM, ctrlHD, ctrlHUOM, ctrlHA) {
    ...
    if ($('[id*=hdnDateOfBirth]').val() == "DONORREG") {


        if (parseInt(years) > 60 || parseInt(years) < parseInt($('#ctl00_ContentPlaceHolder1_hdnMinimumAge').val())) {
            $(".stoast").toastText("Info", "Please enter years between" + $('#ctl00_ContentPlaceHolder1_hdnMinimumAge').val() + " to 60 years", 2, 3);
            $('#ctl00_ContentPlaceHolder1_newAgeUc_txtYear').val(""); 
            ChkValidation();
            /*ctrlY.value = 0;
    ...
    if (days > 31) {
        $(".stoast").toastText("Info", "Please enter valid Nos. of days", 2, 3);
    ...
    if (month > 12) {
        $(".stoast").toastText("Info", "Please enter valid Nos. of months", 2, 3);
        ctrlM.value = 0; month = 0; return false;
    }
    if (month < 0) { $(".stoast").toastText("Info", "Please enter valid Nos. of months", 2, 3); ctrlD.value = ''; ctrlM.value = 0; month = 0; return false; }
    ...
    if (ctrlDOB.value == new Date().format('dd-MMM-yyyy')) { ctrlDOB.value = ''; }
    //    if (ctrlDOB.value == new Date().format('dd-MMM-yyyy')) {
    //        days = 0; 
    //    }
   /* var ageDay = today_Day - days;*/var ageDay = 1;   var noofDays = caldays(ageMonth, ageYear);
    if (ageDay > noofDays) {
    ...
    if (ctrlY.value != '0') {
        ctrlDOB.value = age;
    } ctrlHY.value = years; ctrlHM.value = month;
...
function onExtendAge() {
    ...
    if (localStorage.getItem("ED") != "" && localStorage.getItem("ED") != undefined && localStorage.getItem("ED") != null) {
        ...
        if ((Age != "0(Y), 0(M), 0(D)") && (Dob != '')) {//Shiva 1-1-2016
            ...
            if (((Age == "(Y), (M), (D)") && (Dob == '')) || ((Age == "(Y): (M): (D)") && (Dob == ''))) {
                Age = ""; Dob = "";
                extendedDisplay.setData(3, 'DOB / Age(Y/M/D) :', Dob + Age);
            }
        }
...
function onTitleValidation() {
    ...
    if (ddlvalue == '7' || ddlvalue == '8' || ddlvalue == '5') {
    ...
    if (ddlvalue == "4" || ddlvalue == "5" || ddlvalue == "6" || ddlvalue == "7" || ddlvalue == "8") {
        ...
        if (years >= 18) {
            $(".stoast").toastText("Info", "Enter valid Age as per yhe Title", 2, 3);
            document.getElementById('ctl00_ContentPlaceHolder1_newAgeUc_txtDob').value = '';
            document.getElementById('ctl00_ContentPlaceHolder1_newAgeUc_txtYear').value = '0';
            document.getElementById('ctl00_ContentPlaceHolder1_newAgeUc_txtMonths').value = '0';
            document.getElementById('ctl00_ContentPlaceHolder1_newAgeUc_txtDay').value = '0';
...
function CheckAge() {
    if ($(_pFix + 'newAgeUc_txtYear').val() == '' || $(_pFix + 'newAgeUc_txtYear').val() == 0) {
        years = 0;
        return false;
    }
    else {
        years = parseInt($(_pFix + 'newAgeUc_txtYear').val());
    }
    Minimum_Age_In_Years = parseInt($(_pFix + 'hdnMinimumAge').val());
    if ((years < Minimum_Age_In_Years) || (years > 60) || (years == 0)) {
        $(".stoast").toastText("Info", "Please enter valid age between 18 to 60 years..!", 2, 3);
        $(_pFix + 'newAgeUc_txtDob').val('');
        $(_pFix + 'newAgeUc_txtDay').val('');
        $(_pFix + 'newAgeUc_txtYear').val('');
        $(_pFix + 'newAgeUc_txtMonths').val('');
        $(_pFix + 'newAgeUc_txtDob').focus();
        return false;
    }
}