...
* --------------------------------------------------------------------
*/
jQuery.fn.daterangepicker = function (settings, cfunction) {
    ...
    var options = jQuery.extend({
        ...
        //extras:
			...
			{ text: 'Previous Year', dateStart: function () { return prevYear('S') }, dateEnd: function () { return prevYear('E') } },
        ...
        presets: {
            ...
            // allDatesBefore: 'All Dates Before(10 Years)',
            //  allDatesAfter: 'All Dates After(15Years)',
        ...
        appendTo: 'body',
        onClose: function () {
            if (rangeInput[0].value == "") {
                rangeInput[0].value = " Click to select date.";
            }
        },
        onOpen: function () {
            if (rangeInput[0].value == " Click to select date.") {
                rangeInput[0].value = "";
            }
        },
        onChange: function () { if(cfunction != undefined ) cfunction(rangeInput); },
    ...
    var datepickerOptions = {
        ...
        onSelect: function () {
            ...
            var resultA = CompareGridDates(rangeA, new Date().format('dd-MMM-yyyy'));
            var resultB = rangeB != undefined ? CompareGridDates(rangeB, new Date().format('dd-MMM-yyyy')) : new Date().format('dd-MMM-yyyy');
            var result = '';
            if (resultA == 'd1>d2') {
                if ($('[id*=hdnDtValidation]').val() != 'N') {
                    alert('Start Date Should not be greaterthan today!');
                    rangeA = new Date().format('dd-MMM-yyyy');
                    rp.find('.range-start').datepicker('setDate', new Date());
                    if (rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active')) {
                        rp.find('.range-end').datepicker('setDate', new Date());
                        rangeB = new Date().format('dd-MMM-yyyy');
                    }
                }
            }
            else if (resultB == 'd1>d2') {
                if ($('[id*=hdnDtValidation]').val() != 'N') {
                    alert('End Date Should not be greaterthan today!');
                    rangeB = new Date().format('dd-MMM-yyyy');
                    rp.find('.range-end').datepicker('setDate', new Date());
                }
            }
            else {
                result = CompareGridDates(rangeA, rangeB);
                if (result == 'd1>d2') {
                    alert('Start date should not be greaterthan the End date!');
                    rp.find('.range-end').datepicker('setDate', new Date());
                    rp.find('.range-start').datepicker('setDate', rp.find('.range-end').datepicker('getDate'));
                }
                else {
                    if (rp.find('.ui-daterangepicker-specificDate').is('.ui-state-active')) {
                        rp.find('.range-end').datepicker('setDate', rp.find('.range-start').datepicker('getDate'));
            ...
            var DschfDt = $('[id*=hdnDtSrchfDt]').prop('id');
            var DschtDt = $('[id*=hdnDtSrchtDt]').prop('id');
            if (DschfDt != null & DschfDt != undefined && DschfDt != '') {
                $('[id*=hdnDtSrchfDt]').val(rangeA);
            }
            if (DschfDt != null & DschfDt != undefined && DschfDt != '') {
                $('[id*=hdnDtSrchtDt]').val(rangeB);
            }
            ...
            if (rangeInput.length == 2) {
                ...
                if (rp.find('.ui-daterangepicker-dateRange').is('.ui-state-active'))
                    rangeInput.val((rangeA != rangeB) ? (result == 'd1>d2') ? rangeB + ' ' + options.rangeSplitter + ' ' + rangeA : rangeA + ' ' + options.rangeSplitter + ' ' + rangeB : rangeA);
                else
    ...
    function prevYear(_d) {
        var _dt = new Date();
        if (_d == 'S') {
            return Date.parse('01-01-' + (_dt.getFullYear() - 1).toString());
        }
        else {
            return Date.parse('31-12-' + (_dt.getFullYear() - 1).toString());
        }
    }
