...
public partial class Private_UserControls_GuaranteeAddressUserControl : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            //_iPatientReg = new EzHms.Services.PatientRegistration();
            _pRegCollection = null;//_iPatientReg.Get_PatientOptions(EzHms.ModelEntity.MasterOptions.RELATIONSHIP);
            ddlERelation.DataSource = _pRegCollection;
            ...
            ddlGERelation.DataSource = _pRegCollection;
        ...
        ucArea.LookupName = "AREA";
        List<object> elements = new List<object>();
        elements.Add("0");
        elements.Add(hdndefaultstate.Value);
        ucArea.PreConditon = elements;
        ucArea.OnBlurRequired = true;//this.ucCity.LookupName = this.ucCountry.LookupName = this.ucState.LookupName = "ADDRESS";
        ...
        this.ucGArea.LookupName = "AREA";
        List<object> elements1 = new List<object>();
        elements1.Add("0");
        elements1.Add(hdndefaultstate.Value);
        ucGArea.PreConditon = elements;
        ucGArea.OnBlurRequired = true;//this.ucCity.LookupName = this.ucCountry.LookupName = this.ucState.LookupName = "ADDRESS";
        //this.ucGArea.GettingDataWithObjects = new LookUp.PagingMethodWithObject(_iLookUpSearch.GetLookUpSearchData); //this.ucCity.GettingDataWithObjects = this.ucState.GettingDataWithObjects = this.ucCountry.GettingDataWithObjects = new Private_UserControls_GenericGrid.PagingMethodWithObject(_iLookUpSearch.GetLookUpSearchData);

    }

    public void BindGuarantorRelations(DataTable dt)
    {
        ddlERelation.DataSource = dt;
        ddlERelation.DataTextField = "PATIENT_RELATIONSHIP_DESC";
        ddlERelation.DataValueField = "PATIENT_RELATIONSHIP_ID";
        ddlERelation.DataBind();
        ddlERelation.Items.Insert(0, new ListItem("--select--", "0"));

        ddlGERelation.DataSource = dt;
        ddlGERelation.DataTextField = "PATIENT_RELATIONSHIP_DESC";
        ddlGERelation.DataValueField = "PATIENT_RELATIONSHIP_ID";
        ddlGERelation.DataBind();
        ddlGERelation.Items.Insert(0, new ListItem("--select--", "0"));
        if (!string.IsNullOrEmpty(_emergency_relation))
            ddlERelation.SelectedValue = _emergency_relation;
